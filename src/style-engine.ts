export const LEVELS = new Set(["lite", "full", "ultra", "master", "roleplay"]);
...
function splitPunctuation(sentence: string): PunctuationResult {
  const m = sentence.match(/^([\s\S]*?)([.!?])?$/);
  return { core: (m && m[1]) || sentence, punct: (m && m[2]) || "." };
}

function compactWords(text: string, level: string): string {
...
function yodaPivot(sentence: string): string | null {
...
function sageInvert(sentence: string, level: string): string {
...
function transformSentence(sentence: string, level: string): string {
  if (level === "lite") return sentence;
...
function enforceLengthBudget(input: string, output: string, level: string): string {
...
function compactSentence(s: string, level: string): string {
...
function preserveCodeBlocks(text: string, fn: (chunk: string) => string): string {
...
export function shouldFallbackPlain(text: string): boolean {
...
export function transformText(text: string, level: string = "full"): TransformResult {
